...
This is Kenx's daily pi setup, packaged as a reusable GitHub install: Vim-style prompt editing, custom TUI themes and status bars, Grok-style three-line agent status, right-side file/todo overlays, interactive choice pickers, context snapshots, and a read-only planning mode.
...
- **Grok-style agent status** — `/switch-agentStatus` shows a compact three-line thinking/executing/reading block with the latest assistant text and current file, while edit/write renderers hide diffs and file contents.
...
Interactive HTML preview: [open the agent status playground](https://htmlpreview.github.io/?https://github.com/kenxcomp/yoyo-pi/blob/main/docs/previews/pi-tui-agent-status.html?v=gr0k-hack-1).
...
| Agent status UI | `/switch-agentStatus <1-4\|0\|status>` | Grok-style three-line status widget for thinking/executing/reading plus compact built-in tool renderers. Edit/write show changed file names only; diffs and file contents are hidden. Preferences are stored under `~/.pi/agent/state/gr0k-hack.json`. |
